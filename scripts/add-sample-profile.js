...
async function addSampleProfile() {
  try {
    ...
    console.log('기존 프로필 데이터를 삭제했습니다.');
...
궁금하신 점이나 필요한 작업이 있으시면 언제든지 편하게 문의하십시오. 신뢰와 기술력으로 보답하겠습니다.`,
    ...
    const result = await profiles.insertOne(sampleProfile);
    console.log('프로필 데이터가 추가되었습니다:', result.insertedId);

    // 추가된 데이터 확인
    const count = await profiles.countDocuments();
    console.log(`현재 총 ${count}개의 프로필이 저장되어 있습니다.`);
  } catch (error) {
