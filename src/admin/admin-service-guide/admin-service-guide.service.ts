...
@Injectable()
export class AdminServiceGuideService {
    private readonly logger = new Logger(AdminServiceGuideService.name)
    ...
    async findOrCreateServiceGuide(): Promise<ServiceGuideDocument> {
        this.logger.log("Attempting to find or create service guide...")
        let serviceGuide = await this.serviceGuideModel
            .findOne({ key: SERVICE_GUIDE_KEY })
            .exec()

        if (serviceGuide) {
            this.logger.log(`Found existing service guide. ID: ${serviceGuide._id}`)
        } else {
            this.logger.log("No service guide found. Creating a new one...")
            ...
            try {
                ...
                this.logger.log("Successfully saved new service guide to the database.")
            } catch (error) {
                this.logger.error("Failed to save new service guide.", error.stack)
    ...
    async updateServiceGuide(
        updateDto: UpdateServiceGuideDto,
    ): Promise<ServiceGuideDocument> {
        const updated = await this.serviceGuideModel
            .findOneAndUpdate({ key: SERVICE_GUIDE_KEY }, { $set: updateDto }, {
                new: true,
                upsert: true,
            })
            .exec()
        ...
        if (!updated) {
            throw new BadRequestException("Service guide data not found.")
        }
        return updated
    }
} 